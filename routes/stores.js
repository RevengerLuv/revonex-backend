      ...
      const formattedProducts = products.map((product, index) => {
        ...
        if (product.images && Array.isArray(product.images) && product.images.length > 0) {
          ...
          images = product.images.map(img => {
            ...
            if (img.startsWith('/')) return `https://revonex-backend.onrender.com${img}`;
            return `https://revonex-backend.onrender.com/${img}`;
          });
        } else if (product.image && typeof product.image === 'string') {
          // Product has single image field
          const img = product.image;
          images = [img.startsWith('http') ? img : `https://revonex-backend.onrender.com${img}`];
        } else {
          // Default image
